...
var state={
  ...
  buildTable()
/////------ title for the table===== thead-------------------------------/
  var title = document.getElementById('titles');
  title.innerHTML +=`<tr>
  <th><h1>no</h1></th><th><h1>name</h1></th><th><h1>email</h1></th>
</tr>`;
//---------------------------------------- logic for page--------------------------
  function pagination(querySet,page,rows){
var trimStart=(page-1)*rows;/// 1-1=0*5=0
var trimEnd=trimStart+rows;//0+5=5
var trimmedData=querySet.slice(trimStart,trimEnd);
//console.log(trimmedData)
var pages=Math.round(querySet.length/rows)//100/5=20 20 sets of pages
//console.log(pages)
return{
 ...
 if (state.page != 1) {
       wrapper.innerHTML = `<button value=${1} class="page btn btn-sm btn-info" class="d-flex justify-content-center"> first</button>
       <button value=${state.page -1} class="page btn btn-sm btn-info" class="d-flex justify-content-center"> previous</button>` + wrapper.innerHTML
    }   

    if (state.page != pages) {
        wrapper.innerHTML += `<button value=${pages} class="page btn btn-sm btn-info" class="d-flex justify-content-center">last</button>
        <button value=${state.page +1} class="page btn btn-sm btn-info" class="d-flex justify-content-center"> next</button>`
    ...
    //--------- t-body is the description--------------------
     ...
     // console.log("data:", data)
      ...
      for(var i=1 in myList){
          //keep in mind ... we use..'template literals to create rows"
        //--------------- adding rows inside t head===description
         var row =`<tr>
          <td><p>${myList[i].id}</p></td>
          <td><p>${myList[i].name}</p></td>
          <td><p>${myList[i].email}</p></td></tr>`;
          var description= document.getElementById('description1');
        description.innerHTML+=row ;
      }
